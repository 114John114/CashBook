		...
		methods: {
		    getData() {
		        uni.getStorage({
		            ...
		            success: res => {
		                ...
		                // 过滤数据，只保留支出类型的数据（type == 0）
		                this.dataList = data.filter(item => item.type == 0);
		                this.total();
		    ...
		    total() {
		        ...
		        if (Object.keys(group).length > 0) {
		            const expendMoney = group[0] ? group[0].reduce((sum, item) => sum + Number(item.money), 0) : 0;
		            this.totalExpend = expendMoney.toFixed(2);
		            this.sumExpend = group[0] ? group[0].length : 0;
		        }
		        this.sumCount = data.length;
		        this.totalbalance = -this.totalExpend; // 只有支出数据
		    },
